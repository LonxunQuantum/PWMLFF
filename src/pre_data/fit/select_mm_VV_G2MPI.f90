program linear_VV_G2MPI
       ...
       real*8,allocatable,dimension(:,:) :: S,S_tmp


       real*8,allocatable,dimension(:) :: work,BB,BB_tmp
...
! liuliping, end, all .r .x file should be invoke out of fread_dfeat
       ...
       inode=inode+1
        ...
        call mpi_bcast(include3,1,MPI_INTEGER,0,MPI_COMM_WORLD,ierr)
       ...
       ii=num_case/nnodes
       num_case=ii*nnodes   ! make it dividable
...
!ccccccccccccccc  two different ways to select the test cases
       ...
       ntestM=0
       do i=1,num_case
       ntestM=ntestM+iselect(i)
       enddo
       ntest=ntestM/nnodes+1
       ndiff=ntest*nnodes-ntestM
       if(ndiff.gt.0) then
       ii=0
       do i=1,num_case
       if(iselect(i).eq.0) then
       iselect(i)=1
       ii=ii+1
       if(ii.eq.ndiff) exit
       endif
       ...
       if(inode.eq.1) then
       write(6,*) "ntrain,ntest=", num_case-ntestM,ntestM
       endif

       ncaseM=num_case-ntestM
       ...
       ii1=ii1+1
       if(mod(ii1-1,nnodes)+1.eq.inode) then
       itest=itest+1
       ...
       ii2=ii2+1
       if(mod(ii2-1,nnodes)+1.eq.inode) then
       itrain=itrain+1
...
!cccc distribute the cases
       if(itest.ne.ntest.or.itrain.ne.ncase) then
        write(6,*) "something wrong", itest,ntest,itrain,ncase
        stop
       endif
...
!cccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc
       ...
       allocate(S_tmp(num_ref,num_ref))
       call mpi_allreduce(S,S_tmp,num_ref**2,MPI_REAL8,MPI_SUM, MPI_COMM_WORLD,ierr)
       S=S_tmp
       deallocate(S_tmp)
       ...
       allocate(BB_tmp(num_ref))
       call mpi_allreduce(BB,BB_tmp,num_ref,MPI_REAL8,MPI_SUM, MPI_COMM_WORLD,ierr)
       BB=BB_tmp
       deallocate(BB_tmp)
!cccccccccccc
...
!cccccccccccccccccccccccccccccccccccccccccccccccccc
       ...
       enddo
       call mpi_allreduce(diff1,diff1tmp,1,MPI_REAL8,MPI_SUM, MPI_COMM_WORLD,ierr)
       diff1=diff1tmp
       call mpi_allreduce(diff2,diff2tmp,1,MPI_REAL8,MPI_SUM, MPI_COMM_WORLD,ierr)
       diff2=diff2tmp
       call mpi_allreduce(diff4,diff4tmp,1,MPI_REAL8,MPI_SUM, MPI_COMM_WORLD,ierr)
       diff4=diff4tmp


       diff1a=(diff1/ncaseM)
       diff2a=dsqrt(diff2/ncaseM)
       diff4a=dsqrt(dsqrt(diff4/ncaseM))
!       write(6,*) "iloop",iloop1,iloop2
       ...
       diff4=diff4tmp
       diff1t=(diff1/ntestM)
       diff2t=dsqrt(diff2/ntestM)
       diff4t=dsqrt(dsqrt(diff4/ntestM))
!       write(6,*) "iloop",iloop1,iloop2
       if(inode.eq.1) then
...
!ccc  At this state, iflag_selected(kkk0).eq.1, selected
       ...
       call mpi_allreduce(sum1,sum1tmp,1,MPI_REAL8,MPI_SUM, MPI_COMM_WORLD,ierr)
       sum1=sum1tmp
       call mpi_allreduce(sum2,sum2tmp,1,MPI_REAL8,MPI_SUM, MPI_COMM_WORLD,ierr)
       sum2=sum2tmp

       dE_term(kkk)=abs(sum1**2/sum2/ncaseM)
       endif
       enddo
       enddo
...
! cannot normalize the feature, that is wrong !, keep the feature untouched
       ...
       call mpi_allreduce(sum,sumtmp,1,MPI_REAL8,MPI_SUM, MPI_COMM_WORLD,ierr)
       sum=sumtmp
       ...
       allocate(S_tmp(mm4,mm4))
       call mpi_allreduce(S,S_tmp,mm4**2,MPI_REAL8,MPI_SUM, MPI_COMM_WORLD,ierr)
       S=S_tmp
       deallocate(S_tmp)
...
!cccccccccccccccccccccccccccccccccccccccccccccccccc
       ...
       rewind(10) 
       do i=1,ncase   ! only plotted the first node data
