subroutine fit_VV_force_sub(nimage_jump)
       ...
       real*8,allocatable,dimension(:,:) :: feat_new_group
...
!cccccccccccccccccccccccccccccccccccccccccccccccccccccc
       ...
       allocate(AA_tmp(nfeatNtot,nfeatNtot))
       allocate(BB(nfeatNtot))
       allocate(BB_tmp(nfeatNtot))
...
!cccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc
       ...
       write(6,"('fit,sys,image=',2(i6,1x))") sys,image
...
!      jump from the iat_tmp to the end of  the current loop, after finish AL
...
!     &   -(pi/(2*rad))*cos(yy)*sin(yy))
       ...
       dEdd=4*(w22F_1*(-12*(rad/dd)**12/dd*cos(yy)**2 -(pi/(2*rad))*cos(yy)*sin(yy)*(rad/dd)**12) +W22F_2*(-6*(rad/dd)**6/dd*cos(yy)**2 -(pi/(2*rad))*cos(yy)*sin(yy)*(rad/dd)**6))
...
!cccccccccccccccccccccccccccccccccccccccccccccccc
       ...
       allocate(feat_new_group(nfeatNtot,natom))
...
!cccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc
       ...
       sum=sum+fact
       do j=1,num_ref(itype)  ! note, here different from mm(itype)
       ...
       enddo
!cccccccccccccccccccccccccccccccccccccccccccccc
!       call dgemm('N','T',nfeatN,nfeatN,num(itype),
!     & 1.d0,feat_new_type(1,1,itype),num_refm,feat_new_type(1,1,itype),
!     &  num_refm,
!     & 0.d0,AA_type(1,1,itype),num_refm)


       call dgemm('N','T',nfeatNtot,nfeatNtot,natom, 1.d0,feat_new_group,nfeatNtot,feat_new_group,nfeatNtot, 0.d0,AA_tmp,nfeatNtot) 


       do j=1,nfeatNtot
       sum=0.d0
       do i=1,natom
       sum=sum+energy_group(i)*feat_new_group(j,i)
       enddo
       BB_tmp(j)=sum
       enddo
...
! I will assume, beyond the actual neigh, everything is zero
       ...
       j2=idd(3,kkk,itype)
       id2=idd(4,kkk,itype)
       dfeat_new(kkk+nfeat2(itype),i,jj,:)=dfeat_ext2(ii,j1,id1)* feat_ext2(ii,j2,id2)*dfeat2(j1,i,jj,:)+feat_ext2(ii,j1,id1) *dfeat_ext2(ii,j2,id2)*dfeat2(j2,i,jj,:)
       elseif(idd(0,kkk,itype).eq.3) then
       j1=idd(1,kkk,itype)
       j2=idd(2,kkk,itype)
       j3=idd(3,kkk,itype)
       dfeat_new(kkk+nfeat2(itype),i,jj,:)=dfeat2(j1,i,jj,:)* feat_ext3(ii,j2,1)*feat_ext3(ii,j3,1)+ feat_ext3(ii,j1,1)*dfeat2(j2,i,jj,:)*feat_ext3(ii,j3,1)+ feat_ext3(ii,j1,1)*feat_ext3(ii,j2,1)*dfeat(j3,i,jj,:)
       endif
       enddo
       enddo
       enddo
...
!cccc Now, we have the new features, we need to calculate the distance to reference state
       ...
       do itype=1,ntype
       do j=1,num_ref(itype)
       VV(j+nfeatNi(itype),(i-1)*3+1)=SS(j,i,1,itype) 
       VV(j+nfeatNi(itype),(i-1)*3+2)=SS(j,i,2,itype) 
       VV(j+nfeatNi(itype),(i-1)*3+3)=SS(j,i,3,itype) 
       enddo
       enddo
       enddo
    

       call dgemm('N','T',nfeatNtot,nfeatNtot,3*natom,weight_F, VV,nfeatNtot,VV,nfeatNtot,1.d0,AA,nfeatNtot)
       ...
       do i=1,natom
       do ixyz=1,3
       sum=sum+force(ixyz,i)*VV(j+nfeatNi(itype),(i-1)*3+ixyz)
       enddo
       enddo

       BB(j+nfeatNi(itype))=BB(j+nfeatNi(itype))+sum*weight_F
       enddo
       enddo
...
!        enddo
        ...
        AA(k1+iii1,k2+iii2)=AA(k1+iii1,k2+iii2) + feat22_type(k1,itype1)*feat22_type(k2,itype2)*weight_E0
        enddo
        enddo
        enddo
        enddo
...
!ccccccccccccccccccccccccccccccccccccccccccccccccccccccccc
       call dgesv(nfeatNtot,1,AA,nfeatNtot,ipiv,BB, nfeatNtot,info)  
!cccccccccccccccccccccccccccccccccccccccccccccccccc
