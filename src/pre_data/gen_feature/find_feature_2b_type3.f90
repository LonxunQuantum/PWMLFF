subroutine find_feature_2b_type3(natom,itype_atom,Rc,n2b, &
   ...
   do iat=1,natom
      ...
      dR_neigh_alltype(:,1,iat)=0.d0      ! center atom is itself & vector is zero
...
subroutine find_feature_2b_type3_type_embedding(natom,itype_atom,Rc,n2b, &
   num_neigh,list_neigh, &
   dR_neigh,iat_neigh,ntype,grid2_2, &
   feat_all,dfeat_allR,nfeat0m,m_neigh,n2bm,nfeat_atom, normalization_coeff)
   implicit none
   integer ntype
   integer natom,n2b(ntype)
   integer m_neigh
...
3000 continue
   ...
   do 5000 iat=1,natom
      ...
      do itype = 1, ntype
         do k = 1, n2b(itype0)
            feat2(k,itype,iat) = feat2(k,itype,iat) * normalization_coeff(itype)
            dfeat2(k,itype,iat,1:nneigh,:) = dfeat2(k,itype,iat,1:nneigh,:) * normalization_coeff(itype)
         enddo
      enddo
      ...
      do k=1,n2b(itype0)
         ...
         do itype=1,ntype
            ! feat_all(num,iat)=feat2(k,itype,iat)
            feat_all(k,iat)=feat_all(k,iat) + feat2(k,itype,iat)
            if (abs(feat_all(num,iat)).lt.1E-38) then
               feat_all(num,iat)=0.d0
            endif
            ! dfeat_all(num,iat,1:nneigh,:)=dfeat2(k,itype,iat,1:nneigh,:)
            dfeat_all(k,iat,1:nneigh,:)=dfeat_all(k,iat,1:nneigh,:)+dfeat2(k,itype,iat,1:nneigh,:)
         enddo
      enddo
...
5000 continue
   ...
   return
end subroutine find_feature_2b_type3_type_embedding