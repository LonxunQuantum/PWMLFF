...
class SNESOptimizer(Optimizer):
    def __init__(
        ...
        self.lambda_1 = self.opt_param_nums**0.5 / 1000 if self.input_param.optimizer_param.lambda_1 == -1 else\
            self.input_param.optimizer_param.lambda_1
        self.lambda_2 = self.opt_param_nums**0.5 / 1000 if self.input_param.optimizer_param.lambda_2 == -1 else\
        ...
        self.eta_s = (3 + math.log(self.opt_param_nums)) / (5 * self.opt_param_nums**0.5) if self.input_param.optimizer_param.eta_s is None \
            else self.input_param.optimizer_param.eta_s

    def __init_utility(self):
        utility = []
        for n in range(0, self.population):
            utility.append(max(0.0, math.log(self.population*0.5 + 1.0) - math.log(n + 1.0)))
        for n in range(0, self.population):
            utility[n] = utility[n] / sum(utility) - 1.0/self.population
        return utility
    ...
    def update_params(
        ...
        del_m_J = torch.sum(self.u_k[sorted_indices].unsqueeze(-1) * r_k, dim=1)
        del_s_J = torch.sum(self.u_k[sorted_indices].unsqueeze(-1) * (r_k*r_k -1), dim=1)
        self.m = self.m + self.eta_m * (self.s * del_m_J)
        self.s = self.s * torch.exp(self.eta_s_half * del_s_J)
