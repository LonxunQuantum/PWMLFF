...
# check_pytorch_installed && check_pytorch_version && check_pytorch_cuda_version

# 检查 PyTorch 相关信息
if check_pytorch_installed && check_pytorch_version; then
    if check_pytorch_cuda_version; then
        # 如果 PyTorch 使用 CUDA，则检查 CUDA 版本
        check_cuda_version
        check_nvcc
    fi
fi
