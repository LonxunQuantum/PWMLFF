...
def nep_test(input_json: json, cmd:str):
    ...
    try:
        ...
        nep_content, model_atom_type, atom_names = extract_model(model_load_path, togpumd=False)
    ...
    # nep_trainer.nep_inference(model_load_path)
    nep_trainer.multi_process_nep_inference(model_load_path)
...
def toneplmps(cmd_list:list[str]):
    infos = "\n\nThis cmd is used to convert the nep_model.ckpt trained by PWMLFF to nep.txt for lammps!\n"
    ...
    infos += "    'PWMLFF toneplmps nep_model.ckpt'\n"
    # print(infos)
