...
def conver_to_dp_torch2_version(model_file, atom_type, data, format, savename, save_dir, data_dict=None):
    cpkt = torch.load(model_file, map_location="cpu")
    state_dict_list = list(cpkt['state_dict'].keys())
    ...
    if 'embedding_net.0.weights.weight0' in state_dict_list[0]:
        model_dict, emb_net_size, fit_net_size, atom_num, M2 = read_structure_dp(cpkt)
    else:
    ...
    data_dict = convert_to_dp(model_dict, input_json, savename, save_dir, cpkt['epoch'], data_dict)
    return data_dict

def convert_to_dp(model_dict:dict, input_json:dict, savename:str, save_dir:str, epoch:int, data_dict=None):
    ...
    if 'embedding_net.0.weights.weight0' in state_dict_list[0]:# 
        for layer in state_dict_list:
            ...
            if "embedding_net.0" in layer:
                if ".weights." in layer:
                    emb_net_size.append(cpkt['state_dict'][layer].shape[1])
            if "fitting_net.0" in layer:
                if ".weights." in layer:
                    fit_net_size.append(cpkt['state_dict'][layer].shape[1])
            M2 = int(cpkt['state_dict']["fitting_net.0.weights.weight0"].shape[0]/emb_net_size[-1])
            new_layer_name = "{}.{}.{}.{}.{}".format(net_name, net_id, 'layers', layer_id, layer_name)
            model_dict[new_layer_name] = cpkt['state_dict'][layer]
