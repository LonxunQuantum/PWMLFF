...
{ //l = 1
  ...
  for(int k=0; k < n_base_angular; k++) {
    k_idx = k * 4;
    // 左边项 rij
    f12k[k_idx + 0] += C3B[0] * fn12[k] * rij_blm[0] * fnp * blm[0] + 
             2.0 * C3B[1] * fn12[k] * rij_blm[1] * fnp * blm[1] + 
             2.0 * C3B[2] * fn12[k] * rij_blm[2] * fnp * blm[2];
    // 右边项
    f12k[k_idx + 0] += s[0] * (fnp12[k] * rij_Lsq - fn12[k] * rij_L2sq) * blm[0] + 
                 2.0 * s[1] * (fnp12[k] * rij_Lsq - fn12[k] * rij_L2sq) * blm[1] + 
                 2.0 * s[2] * (fnp12[k] * rij_Lsq - fn12[k] * rij_L2sq) * blm[2]; // 后项 dblm/drij L=1 时 为0
    f12k[k_idx + 0] *= 2.0 * Fp * scd_r12[0];// 可以考虑移出去

    // xij
    f12k[k_idx + 1] += 2.0 * C3B[1] * fn12[k] * rij_blm[1] * fn; // dblm/dxij b10 b12 为0
    f12k[k_idx + 1] += 2.0 * s[1] * fn12[k] * rij_Lsq; // dblm/dxij b10 b12 为0
    f12k[k_idx + 1] *= 2.0 * Fp * scd_r12[1];

    // yij
    f12k[k_idx + 2] += 2.0 * C3B[2] * fn12[k] * rij_blm[2] * fn;
    f12k[k_idx + 2] += 2.0 * s[2] * fn12[k] * rij_Lsq;
    f12k[k_idx + 2] *= 2.0 * Fp * scd_r12[2];

    // zij
    f12k[k_idx + 3] += C3B[0] * fn12[k] * rij_blm[0] * fn;
    f12k[k_idx + 3] += s[0] * fn12[k] * rij_Lsq;
    f12k[k_idx + 3] *= 2.0 * Fp * scd_r12[3];
  }
  // if (n1==0 and n2==0){
  ...
  )
{
  ...
  for(int k=0; k < n_base_angular; k++) {
    k_idx = k * 4;
    // 左边项 rij
    f12k[k_idx + 0] +=  C3B[3] * fn12[k] * rij_blm[3] * (fnp * blm[3] + fn * dblm_r[3]) + 
                  2.0 * C3B[4] * fn12[k] * rij_blm[4] * fnp * blm[4] + 
                  2.0 * C3B[5] * fn12[k] * rij_blm[5] * fnp * blm[5] + 
                  2.0 * C3B[6] * fn12[k] * rij_blm[6] * fnp * blm[6] +
                  2.0 * C3B[7] * fn12[k] * rij_blm[7] * fnp * blm[7]; 
    // 右边项
    f12k[k_idx + 0] += s[0] * ((fnp12[k] * rij_Lsq - 2.0 * fn12[k] * rij_L2sq) * blm[3] + fn12[k] * rij_Lsq * dblm_r[3]) +  
    ...
    // 左边项 xij
    f12k[k_idx + 1] += 
                  2.0 * C3B[4] * fn12[k] * rij_blm[4] * fn * dblm_x[4] + 
                  2.0 * C3B[6] * fn12[k] * rij_blm[6] * fn * dblm_x[6] +
                  2.0 * C3B[7] * fn12[k] * rij_blm[7] * fn * dblm_x[7];     
    // 右边项
    f12k[k_idx + 1] += 
            2.0 * s[1] * fn12[k] * rij_Lsq * dblm_x[1] +
            2.0 * s[3] * fn12[k] * rij_Lsq * dblm_x[3] +
            2.0 * s[4] * fn12[k] * rij_Lsq * dblm_x[4];
    f12k[k_idx + 1] *= 2.0 * Fp * scd_r12[1];

    // 左边项 yij
    f12k[k_idx + 2] += 
                  2.0 * C3B[5] * fn12[k] * rij_blm[5] * fn * dblm_y[5] + 
                  2.0 * C3B[6] * fn12[k] * rij_blm[6] * fn * dblm_y[6] +
                  2.0 * C3B[7] * fn12[k] * rij_blm[7] * fn * dblm_y[7];     
    // 右边项
    f12k[k_idx + 2] += 
            2.0 * s[2] * fn12[k] * rij_Lsq * dblm_y[2] + 
            2.0 * s[3] * fn12[k] * rij_Lsq * dblm_y[3] +
            2.0 * s[4] * fn12[k] * rij_Lsq * dblm_y[4];    
    f12k[k_idx + 2] *= 2.0 * Fp * scd_r12[2];
    
    // 左边项 zij
    f12k[k_idx + 3] +=  C3B[3] * fn12[k] * rij_blm[3] * fn * dblm_z[3] + 
                  2.0 * C3B[4] * fn12[k] * rij_blm[4] * fn * dblm_z[4] + 
                  2.0 * C3B[5] * fn12[k] * rij_blm[5] * fn * dblm_z[5];     
    // 右边项
    f12k[k_idx + 3] +=  s[0] * fn12[k] * rij_Lsq * dblm_z[0] +  
                  2.0 * s[1] * fn12[k] * rij_Lsq * dblm_z[1] +
                  2.0 * s[2] * fn12[k] * rij_Lsq * dblm_z[2];
    f12k[k_idx + 3] *= 2.0 * Fp * scd_r12[3];
  }
  // if (n1==0 and n2==0){
...
)
{
  ...
  for(int k=0; k < n_base_angular; k++) {
    ...
    // drij
    // d0
    f12k[k_idx + 0] += 4.0 * C5B[0] * (3.0 * s2[0] * dnlm_dc[0] * dnlm_drij[0] + s2[0] * s[0] * dnlm_drij_dc[0]);
    // d1
    ...
    f12k[k_idx + 0] += 2.0 * C5B[1] * (
      dnlm_dc[0] * dnlm_drij[0] * (s2[1] + s2[2]) + s[0] * dnlm_drij_dc[0] * (s2[1] + s2[2]) + 
      s[0] * dnlm_drij[0] * ds1s2_c + 2.0 * s[0] * dnlm_dc[0] * ds1s2 + s2[0] * d_tmp);
    // d2
    f12k[k_idx + 0] += 4.0 * C5B[2] * (ds1s2_c * ds1s2 + (s2[1] + s2[2]) * d_tmp);
    f12k[k_idx + 0] *= 2.0 * Fp * scd_r12[0];

    // dxij
    // d0
    f12k[k_idx + 1] += 4.0 * C5B[0] * (3.0 * s2[0] * dnlm_dc[0] * dnlm_dxij[0] + s2[0] * s[0] * dnlm_dxij_dc[0]);
    // d1
    ...
    f12k[k_idx + 1] += 2.0 * C5B[1] * (
      dnlm_dc[0] * dnlm_dxij[0] * (s2[1] + s2[2]) + s[0] * dnlm_dxij_dc[0] * (s2[1] + s2[2]) + 
      s[0] * dnlm_dxij[0] * ds1s2_c + 2.0 * s[0] * dnlm_dc[0] * ds1s2 + s2[0] * d_tmp);
    // d2
    f12k[k_idx + 1] += 4.0 * C5B[2] * (ds1s2_c * ds1s2 + (s2[1] + s2[2]) * d_tmp);
    f12k[k_idx + 1] *= 2.0 * Fp * scd_r12[1];

    // dyij
    // d0
    f12k[k_idx + 2] += 4.0 * C5B[0] * (3.0 * s2[0] * dnlm_dc[0] * dnlm_dyij[0] + s2[0] * s[0] * dnlm_dyij_dc[0]);
    // d1
    ...
    f12k[k_idx + 2] += 2.0 * C5B[1] * (
      dnlm_dc[0] * dnlm_dyij[0] * (s2[1] + s2[2]) + s[0] * dnlm_dyij_dc[0] * (s2[1] + s2[2]) + 
      s[0] * dnlm_dyij[0] * ds1s2_c + 2.0 * s[0] * dnlm_dc[0] * ds1s2 + s2[0] * d_tmp);
    // d2
    f12k[k_idx + 2] += 4.0 * C5B[2] * (ds1s2_c * ds1s2 + (s2[1] + s2[2]) * d_tmp);
    f12k[k_idx + 2] *= 2.0 * Fp * scd_r12[2];

    // dzij
    // d0
    f12k[k_idx + 3] += 4.0 * C5B[0] * (3.0 * s2[0] * dnlm_dc[0] * dnlm_dzij[0] + s2[0] * s[0] * dnlm_dzij_dc[0]);
    // d1
  ...
  double *f12k)
{
  ...
  for(int k=0; k < n_base_angular; k++) {
    k_idx = k * 4;
    // 左边项 rij
    f12k[k_idx + 0] +=  C3B[8]  * fn12[k]  * rij_blm[8] * (fnp * blm[8]  + fn * dblm_r[8]) + 
                  2.0 * C3B[9]  * fn12[k]  * rij_blm[9] * (fnp * blm[9]  + fn * dblm_r[9]) + 
                  2.0 * C3B[10] * fn12[k] * rij_blm[10] * (fnp * blm[10] + fn * dblm_r[10]); 
                // + 2.0 * C3B[11] * fn12[k] * rij_blm[11] * (fnp * blm[11] + fn * 0) +
                //   2.0 * C3B[12] * fn12[k] * rij_blm[12] * (fnp * blm[12] + fn * 0) +
                //   2.0 * C3B[13] * fn12[k] * rij_blm[13] * (fnp * blm[13] + fn * 0) +
                //   2.0 * C3B[14] * fn12[k] * rij_blm[14] * (fnp * blm[14] + fn * 0); 
    // 右边项
    f12k[k_idx + 0] += s[0] * ((fnp12[k] * rij_Lsq - 3.0 * fn12[k] * rij_L2sq) * blm[8] + fn12[k] * rij_Lsq * dblm_r[8]) +
            ...
            // 2.0 * s[3] * ((fnp12[k] * rij_Lsq - 3.0 * fn12[k] * rij_L2sq) * blm[11] + fn * 0) +
            // 2.0 * s[4] * ((fnp12[k] * rij_Lsq - 3.0 * fn12[k] * rij_L2sq) * blm[12] + fn * 0) +
            // 2.0 * s[5] * ((fnp12[k] * rij_Lsq - 3.0 * fn12[k] * rij_L2sq) * blm[13] + fn * 0) +
            // 2.0 * s[6] * ((fnp12[k] * rij_Lsq - 3.0 * fn12[k] * rij_L2sq) * blm[14] + fn * 0); 
    f12k[k_idx + 0] *= 2.0 * Fp * scd_r12[0];// 可以考虑移出去

    // 左边项 xij
    f12k[k_idx + 1] += 
                        // C3B[8]  * fn12[k] * rij_blm[8]  * fn * dblm_x[8] +  
                  2.0 * C3B[9]  * fn12[k] * rij_blm[9]  * fn * dblm_x[9] + 
                  // 2.0 * C3B[10] * fn12[k] * rij_blm[10] * fn * dblm_x[10] + 
                  2.0 * C3B[11] * fn12[k] * rij_blm[11] * fn * dblm_x[11] + 
                  2.0 * C3B[12] * fn12[k] * rij_blm[12] * fn * dblm_x[12] + 
                  2.0 * C3B[13] * fn12[k] * rij_blm[13] * fn * dblm_x[13] +
                  2.0 * C3B[14] * fn12[k] * rij_blm[14] * fn * dblm_x[14];     
    // 右边项
    f12k[k_idx + 1]  += 
                        // s[0] * fn12[k] * rij_Lsq * dblm_x[8] + 0.0 +
                  2.0 * s[1] * fn12[k] * rij_Lsq * dblm_x[9] + 
                  // 2.0 * s[2] * fn12[k] * rij_Lsq * dblm_x[10] + 0.0 +
    ...
    // 左边项 yij
    f12k[k_idx + 2] +=  
                  //       C3B[8] * fn12[k] * rij_blm[8]  * fn * dblm_y[8] +  
                  // 2.0 * C3B[9] * fn12[k] * rij_blm[9]  * fn * dblm_y[9] + 
                  2.0 * C3B[10] * fn12[k] * rij_blm[10] * fn * dblm_y[10] +
                  2.0 * C3B[11] * fn12[k] * rij_blm[11] * fn * dblm_y[11] +
                  2.0 * C3B[12] * fn12[k] * rij_blm[12] * fn * dblm_y[12] +
                  2.0 * C3B[13] * fn12[k] * rij_blm[13] * fn * dblm_y[13] +
                  2.0 * C3B[14] * fn12[k] * rij_blm[14] * fn * dblm_y[14];     
    // 右边项
    f12k[k_idx + 2] += 
                        // s[0] * fn12[k] * rij_Lsq * dblm_y[8] + 0.0 +
    ...
    f12k[k_idx + 2] *= 2.0 * Fp * scd_r12[1];

    // 左边项 yij
    f12k[k_idx + 3] +=   C3B[8] * fn12[k] * rij_blm[8]  * fn * dblm_z[8] +  
                  2.0 * C3B[9] * fn12[k] * rij_blm[9]   * fn * dblm_z[9] +  
                  2.0 * C3B[10] * fn12[k] * rij_blm[10] * fn * dblm_z[10] +
                  2.0 * C3B[11] * fn12[k] * rij_blm[11] * fn * dblm_z[11] +
                  2.0 * C3B[12] * fn12[k] * rij_blm[12] * fn * dblm_z[12];
                  // 2.0 * C3B[13] * fn12[k] * rij_blm[13] * fn  * dblm_z[13] + * 0.0 +
                  // 2.0 * C3B[14] * fn12[k] * rij_blm[14] * fn  * dblm_z[14] + * 0.0;     
    // 右边项
    f12k[k_idx + 3]  += s[0] * fn12[k] * rij_Lsq * dblm_z[8] + 
  ...
  double *f12k)
{
  ...
  for(int k=0; k < n_base_angular; k++) {
    k_idx = k * 4;
    // 左边项 rij
    f12k[k_idx + 0] +=  C3B[15] * fn12[k] * rij_blm[15] * (fnp * blm[15] + fn12[k] * dblm_r[15]) + 
                  2.0 * C3B[16] * fn12[k] * rij_blm[16] * (fnp * blm[16] + fn12[k] * dblm_r[16]) + 
                  2.0 * C3B[17] * fn12[k] * rij_blm[17] * (fnp * blm[17] + fn12[k] * dblm_r[17]) + 
                  2.0 * C3B[18] * fn12[k] * rij_blm[18] * (fnp * blm[18] + fn12[k] * dblm_r[18]) + 
                  2.0 * C3B[19] * fn12[k] * rij_blm[19] * (fnp * blm[19] + fn12[k] * dblm_r[19]) + 
                  2.0 * C3B[20] * fn12[k] * rij_blm[20] * (fnp * blm[20]) +
                  2.0 * C3B[21] * fn12[k] * rij_blm[21] * (fnp * blm[21]) +
                  2.0 * C3B[22] * fn12[k] * rij_blm[22] * (fnp * blm[22]) +
                  2.0 * C3B[23] * fn12[k] * rij_blm[23] * (fnp * blm[23]); 
    // 右边项
    f12k[k_idx + 0] += s[0] * ((fnp12[k]  * rij_Lsq - 3.0 * fn12[k] * rij_L2sq) * blm[15] + fn12[k] * dblm_r[15]) +  
                  2.0 * s[1] * ((fnp12[k] * rij_Lsq - 3.0 * fn12[k] * rij_L2sq) * blm[16] + fn12[k] * dblm_r[16]) +
                  2.0 * s[2] * ((fnp12[k] * rij_Lsq - 3.0 * fn12[k] * rij_L2sq) * blm[17] + fn12[k] * dblm_r[17]) +
                  2.0 * s[3] * ((fnp12[k] * rij_Lsq - 3.0 * fn12[k] * rij_L2sq) * blm[18] + fn12[k] * dblm_r[18]) +
                  2.0 * s[4] * ((fnp12[k] * rij_Lsq - 3.0 * fn12[k] * rij_L2sq) * blm[19] + fn12[k] * dblm_r[19]) +
                  2.0 * s[5] * ((fnp12[k] * rij_Lsq - 3.0 * fn12[k] * rij_L2sq) * blm[20]) +
                  2.0 * s[6] * ((fnp12[k] * rij_Lsq - 3.0 * fn12[k] * rij_L2sq) * blm[21]) +
                  2.0 * s[7] * ((fnp12[k] * rij_Lsq - 3.0 * fn12[k] * rij_L2sq) * blm[22]) +
                  2.0 * s[8] * ((fnp12[k] * rij_Lsq - 3.0 * fn12[k] * rij_L2sq) * blm[23]); 
    f12k[k_idx + 0] *= 2.0 * Fp * scd_r12[0];// 可以考虑移出去

    // 左边项 xij
    f12k[k_idx + 1] +=  C3B[15] * fn12[k] * rij_blm[15] * fn12[k] * dblm_x[15] + 
                  2.0 * C3B[16] * fn12[k] * rij_blm[16] * fn12[k] * dblm_x[16] + 
                  2.0 * C3B[17] * fn12[k] * rij_blm[17] * fn12[k] * dblm_x[17] + 
                  2.0 * C3B[18] * fn12[k] * rij_blm[18] * fn12[k] * dblm_x[18] + 
                  2.0 * C3B[19] * fn12[k] * rij_blm[19] * fn12[k] * dblm_x[19] + 
                  2.0 * C3B[20] * fn12[k] * rij_blm[20] * fn12[k] * dblm_x[20] + 
                  2.0 * C3B[21] * fn12[k] * rij_blm[21] * fn12[k] * dblm_x[21] + 
                  2.0 * C3B[22] * fn12[k] * rij_blm[22] * fn12[k] * dblm_x[22] + 
                  2.0 * C3B[23] * fn12[k] * rij_blm[23] * fn12[k] * dblm_x[23];

    // 右边项 xij
    f12k[k_idx + 1] +=  s[0] * fn12[k] * rij_Lsq * dblm_x[15] + 
    ...
    // 左边项 yij
    f12k[k_idx + 2] +=  C3B[15] * fn12[k] * rij_blm[15] * fn12[k] * dblm_y[15] + 
                  2.0 * C3B[16] * fn12[k] * rij_blm[16] * fn12[k] * dblm_y[16] + 
                  2.0 * C3B[17] * fn12[k] * rij_blm[17] * fn12[k] * dblm_y[17] + 
                  2.0 * C3B[18] * fn12[k] * rij_blm[18] * fn12[k] * dblm_y[18] + 
                  2.0 * C3B[19] * fn12[k] * rij_blm[19] * fn12[k] * dblm_y[19] + 
                  2.0 * C3B[20] * fn12[k] * rij_blm[20] * fn12[k] * dblm_y[20] + 
                  2.0 * C3B[21] * fn12[k] * rij_blm[21] * fn12[k] * dblm_y[21] + 
                  2.0 * C3B[22] * fn12[k] * rij_blm[22] * fn12[k] * dblm_y[22] + 
                  2.0 * C3B[23] * fn12[k] * rij_blm[23] * fn12[k] * dblm_y[23];

    // 右边项 yij
    f12k[k_idx + 2] +=  s[0] * fn12[k] * rij_Lsq * dblm_y[15] + 
    ...
    // 左边项 zij
    f12k[k_idx + 3] +=  C3B[15] * fn12[k] * rij_blm[15] * fn12[k] * dblm_z[15] + 
                  2.0 * C3B[16] * fn12[k] * rij_blm[16] * fn12[k] * dblm_z[16] + 
                  2.0 * C3B[17] * fn12[k] * rij_blm[17] * fn12[k] * dblm_z[17] + 
                  2.0 * C3B[18] * fn12[k] * rij_blm[18] * fn12[k] * dblm_z[18] + 
                  2.0 * C3B[19] * fn12[k] * rij_blm[19] * fn12[k] * dblm_z[19] + 
                  2.0 * C3B[20] * fn12[k] * rij_blm[20] * fn12[k] * dblm_z[20] + 
                  2.0 * C3B[21] * fn12[k] * rij_blm[21] * fn12[k] * dblm_z[21] + 
                  2.0 * C3B[22] * fn12[k] * rij_blm[22] * fn12[k] * dblm_z[22] + 
                  2.0 * C3B[23] * fn12[k] * rij_blm[23] * fn12[k] * dblm_z[23];

    // 右边项 zij
    f12k[k_idx + 3] +=  s[0] * fn12[k] * rij_Lsq * dblm_z[15] + 
