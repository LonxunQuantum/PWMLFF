...
void print_gpu_information(void)
{
  ...
  int num_gpus;
  CHECK_CUDA_NEP(cudaGetDeviceCount(&num_gpus));
  printf("number of GPUs = %d\n", num_gpus);

  for (int device_id = 0; device_id < num_gpus; ++device_id) {
    cudaDeviceProp prop;
    CHECK_CUDA_NEP(cudaGetDeviceProperties(&prop, device_id));
  ...
  for (int i = 0; i < num_gpus; i++) {
    ...
    for (int j = 0; j < num_gpus; j++) {
      int can_access;
      if (i != j) {
        CHECK_CUDA_NEP(cudaDeviceCanAccessPeer(&can_access, i, j));
        if (can_access) {
          CHECK_CUDA_NEP(cudaDeviceEnablePeerAccess(j, 0));
