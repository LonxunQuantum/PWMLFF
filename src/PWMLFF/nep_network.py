...
class nep_network:
    def __init__(self, nep_param:InputParam):
        ...
        if self.nep_param.seed is not None:
            random.seed(self.nep_param.seed)
            torch.manual_seed(self.nep_param.seed)
        ...
        if torch.cuda.is_available():
            if self.nep_param.gpu:
                print("Use GPU: {} for training".format(self.nep_param.gpu))
                self.device = torch.device("cuda:{}".format(self.nep_param.gpu))
            else:
        ...
        if self.nep_param.precision == "float32":
    ...
    def generate_data(self):    
        ...
        """
        raw_data_path = self.nep_param.file_paths.raw_path
        datasets_path = os.path.join(self.nep_param.file_paths.json_dir, self.nep_param.file_paths.trainSetDir)
        train_ratio = self.nep_param.train_valid_ratio
        train_data_path = self.nep_param.file_paths.trainDataPath
        valid_data_path = self.nep_param.file_paths.validDataPath
    ...
    def _get_stat(self):
        ...
        if self.nep_param.inference:
            if os.path.exists(self.nep_param.file_paths.model_load_path):
                # load davg, dstd from checkpoint of model
                davg, dstd, atom_map, energy_shift = load_davg_dstd_from_checkpoint(self.nep_param.file_paths.model_load_path)
            elif os.path.exists(self.nep_param.file_paths.model_save_path):
                davg, dstd, atom_map, energy_shift = load_davg_dstd_from_checkpoint(self.nep_param.file_paths.model_save_path)
            else:
                raise Exception("Erorr! Loading model for inference can not find checkpoint: \
                                \nmodel load path: {} \n or model at work path: {}\n"\
                                .format(self.nep_param.file_paths.model_load_path, self.nep_param.file_paths.model_save_path))
    ...
    def load_data(self, energy_shift, max_atom_nums):
        ...
        # Create dataset
        if self.nep_param.inference:
            train_dataset = MovementDataset("train", self.nep_param, energy_shift, max_atom_nums)
            valid_dataset = None
        else:            
            train_dataset = MovementDataset("train", self.nep_param, energy_shift, max_atom_nums)
            valid_dataset = MovementDataset("valid", self.nep_param, energy_shift, max_atom_nums)
        ...
        train_loader = torch.utils.data.DataLoader(
            train_dataset,
            batch_size=self.nep_param.optimizer_param.batch_size,
            shuffle=self.nep_param.data_shuffle,
            num_workers=self.nep_param.workers,   
            pin_memory=True,
    ...
    def get_stat(self):
        if self.davg_dstd_energy_shift is None:
            if os.path.exists(self.nep_param.file_paths.model_load_path) is False:
                raise Exception("ERROR! {} is not exist when get energy shift !".format(self.nep_param.file_paths.model_load_path))
            davg_dstd_energy_shift = load_davg_dstd_from_checkpoint(self.nep_param.file_paths.model_load_path)
        else:
    ...
    def load_model_optimizer(self, energy_shift):
        ...
        model = NEP(self.nep_param, energy_shift)
        model = model.to(self.training_type)

        # optionally resume from a checkpoint
        checkpoint = None
        if self.nep_param.recover_train:
            if self.inference and os.path.exists(self.nep_param.file_paths.model_load_path): # recover from user input ckpt file for inference work
                model_path = self.nep_param.file_paths.model_load_path
            else: # resume model specified by user
                model_path = self.nep_param.file_paths.model_save_path  #recover from last training for training
            if os.path.isfile(model_path):
                print("=> loading checkpoint '{}'".format(model_path))
                if not torch.cuda.is_available():
                    checkpoint = torch.load(model_path,map_location=torch.device('cpu') )
                elif self.nep_param.gpu is None:
                    checkpoint = torch.load(model_path)
                elif torch.cuda.is_available():
                    # Map model to be loaded to specified single gpu.
                    loc = "cuda:{}".format(self.nep_param.gpu)
                    checkpoint = torch.load(model_path, map_location=loc)
                # start afresh
                if self.nep_param.optimizer_param.reset_epoch:
                    self.nep_param.optimizer_param.start_epoch = 1
                else:
                    self.nep_param.optimizer_param.start_epoch = checkpoint["epoch"] + 1
        ...
        if not torch.cuda.is_available():
            ...
            '''
        elif self.nep_param.gpu is not None and torch.cuda.is_available():
            torch.cuda.set_device(self.nep_param.gpu)
            model = model.cuda(self.nep_param.gpu)
        else:
            model = model.cuda()
            # if model.compress_tab is not None:
            #     model.compress_tab.to(device=self.device)
        # optimizer, and learning rate scheduler
        if self.nep_param.optimizer_param.opt_name == "LKF":
            optimizer = LKFOptimizer(
                model.parameters(),
                self.nep_param.optimizer_param.kalman_lambda,
                self.nep_param.optimizer_param.kalman_nue,
                self.nep_param.optimizer_param.block_size
            )
        elif self.nep_param.optimizer_param.opt_name == "GKF":
            optimizer = GKFOptimizer(
                model.parameters(),
                self.nep_param.optimizer_param.kalman_lambda,
                self.nep_param.optimizer_param.kalman_nue
            )
        elif self.nep_param.optimizer_param.opt_name == "ADAM":
            optimizer = optim.Adam(model.parameters(), 
                                   self.nep_param.optimizer_param.learning_rate)
        elif self.nep_param.optimizer_param.opt_name == "SGD":
            optimizer = optim.SGD(
                model.parameters(), 
                self.nep_param.optimizer_param.learning_rate,
                momentum=self.nep_param.optimizer_param.momentum,
                weight_decay=self.nep_param.optimizer_param.weight_decay
            )
        else:
    ...
    def inference(self, davg, dstd, energy_shift, max_atom_nums):
        ...
        = predict(train_loader, model, self.criterion, self.device, self.nep_param)
        end = time.time()
        ...
        if self.nep_param.optimizer_param.train_egroup:
            inference_cout += "Avarage REMSE of RMSE_Egroup: {} \n".format(res_pd['RMSE_Egroup'].mean())
        if self.nep_param.optimizer_param.train_virial:
        ...
        inference_cout += "\nMore details can be found under the file directory:\n{}\n".format(os.path.realpath(self.nep_param.file_paths.test_dir))
        print(inference_cout)

        inference_path = self.nep_param.file_paths.test_dir
    ...
    def train(self,energy_shift, max_atom_nums):
        ...
        train_log = os.path.join(self.nep_param.file_paths.model_store_dir, "epoch_train.dat")
        f_train_log = open(train_log, "w")
        valid_log = os.path.join(self.nep_param.file_paths.model_store_dir, "epoch_valid.dat")
        f_valid_log = open(valid_log, "w")
        # Define the lists based on the training type
        train_lists = ["epoch", "loss"]
        valid_lists = ["epoch", "loss"]
        if self.nep_param.optimizer_param.train_energy:
        ...
        if self.nep_param.optimizer_param.opt_name == "LKF" or self.nep_param.optimizer_param.opt_name == "GKF":
        ...
        for epoch in range(self.nep_param.optimizer_param.start_epoch, self.nep_param.optimizer_param.epochs + 1):
            ...
            if self.nep_param.optimizer_param.opt_name == "LKF" or self.nep_param.optimizer_param.opt_name == "GKF":
                loss, loss_Etot, loss_Etot_per_atom, loss_Force, loss_Ei, loss_egroup, loss_virial, loss_virial_per_atom, Sij_max = train_KF(
                    train_loader, model, self.criterion, optimizer, epoch, self.device, self.nep_param
                )
            else:
                loss, loss_Etot, loss_Etot_per_atom, loss_Force, loss_Ei, loss_egroup, loss_virial, loss_virial_per_atom, real_lr, Sij_max = train(
                    train_loader, model, self.criterion, optimizer, epoch, \
                        self.nep_param.optimizer_param.learning_rate, self.device, self.nep_param
                )
            time_end = time.time()
            ...
            vld_loss, vld_loss_Etot, vld_loss_Etot_per_atom, vld_loss_Force, vld_loss_Ei, val_loss_egroup, val_loss_virial, val_loss_virial_per_atom = valid(
                val_loader, model, self.criterion, self.device, self.nep_param
            )
            ...
            # if not self.dp_params.hvd or (self.dp_params.hvd and hvd.rank() == 0):
            if self.nep_param.file_paths.save_p_matrix:
                save_checkpoint(
                    {
                    "json_file":self.nep_param.to_dict(),
                    "epoch": epoch,
                    ...
                    self.nep_param.file_paths.model_name,
                    self.nep_param.file_paths.model_store_dir,
                )
            else: 
                save_checkpoint(
                    {
    ...
    def evaluate(self,num_thread = 1):
        ...
        import md100
        md100.run_md100(imodel = 5, atom_type = self.nep_param.atom_type, num_process = num_thread) 
    ...
    def run_md(self, init_config = "atom.config", md_details = None, num_thread = 1,follow = False):
        ...
        f.write('%d\n' % len(self.nep_param.atom_type)) 
        
        for i in range(len(self.nep_param.atom_type)):
            f.write('%d %d\n' % (self.nep_param.atom_type[i], 2*self.nep_param.atom_type[i]))
        f.close()    
