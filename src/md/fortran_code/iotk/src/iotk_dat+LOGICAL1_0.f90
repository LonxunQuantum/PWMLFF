...
call iotk_error_msg(ierrl,"CVS Revision: 1.27 ")
    ...
    call iotk_write_begin(unit,iotk_tolower("LOGICAL"),lattr,ierr=ierrl,new_line=.false.)
  end if
!->
  allocate(dattmp(1))
     dattmp(1) = dat
...
call iotk_error_msg(ierrl,"CVS Revision: 1.27 ")
     ...
     write(lunit,fmt=trim(iotk_wfmt("LOGICAL",kind(dattmp),lcolumns,-1,lsep)),advance='no',iostat=iostat) dattmp(1)
     if(iostat/=0) then
      call iotk_error_issue(ierrl,"iotk_write_dat",__FILE__,__LINE__)
call iotk_error_msg(ierrl,"CVS Revision: 1.27 ")
      goto 1
...
!-<
  if (qe_syntax) then
    call iotk_write_end(unit,iotk_tolower("LOGICAL"),indentation=.false.,ierr=ierrl)
    if(ierrl/=0) then
      call iotk_error_issue(ierrl,"iotk_write_dat",__FILE__,__LINE__)
call iotk_error_msg(ierrl,"CVS Revision: 1.27 ")
      goto 1
...
!-<
    if (qe_syntax) then 
      call iotk_scan_end(unit,iotk_tolower("LOGICAL"),ierr=ierrl2) 
       if(ierrl2/=0) then
...
call iotk_error_msg(ierrl,"CVS Revision: 1.27 ")
  ...
  call iotk_scan_begin(unit,iotk_tolower("LOGICAL"),lattr,found=foundl,ierr=ierrl)
  if(.not. foundl) goto 1
...
call iotk_error_msg(ierrl,"CVS Revision: 1.27 ")
    ...
    call iotk_write_begin(unit,iotk_tolower("LOGICAL"),lattr,ierr=ierrl,new_line=.true.)
  end if
!->
...
!-<
  if (qe_syntax) then
    call iotk_write_end(unit,iotk_tolower("LOGICAL"),indentation=.true.,ierr=ierrl)
    if(ierrl/=0) then
      call iotk_error_issue(ierrl,"iotk_write_dat",__FILE__,__LINE__)
call iotk_error_msg(ierrl,"CVS Revision: 1.27 ")
      goto 1
