...
call iotk_error_msg(ierrl,"CVS Revision: 1.27 ")
    ...
    call iotk_write_begin(unit,iotk_tolower("INTEGER"),lattr,ierr=ierrl,new_line=.false.)
  end if
!->
  allocate(dattmp(1))
     dattmp(1) = dat
...
!-<
  if (qe_syntax) then
    call iotk_write_end(unit,iotk_tolower("INTEGER"),indentation=.false.,ierr=ierrl)
    if(ierrl/=0) then
      call iotk_error_issue(ierrl,"iotk_write_dat",__FILE__,__LINE__)
call iotk_error_msg(ierrl,"CVS Revision: 1.27 ")
      goto 1
