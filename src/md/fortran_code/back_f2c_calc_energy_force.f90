subroutine f2c_calc_energy_force(i_model_lvn, n_atom, type_atom, lat,&
    x_frac, e_atom_out, f_atom_out) bind(c,name="f2c_calc_energy_force")
    use iso_c_binding
    use mod_md
    use mod_mpi
    !use mod_control, only : MCTRL_iMD, MCTRL_AL,MCTRL_output_nstep
    use mod_control
    use mod_data  ! xatom, iatom, natom, AL, f_atom, e_atom and etc
...
!        call readf_xatom_new(iMD,ntype_mass,itype_mass,mass_type)
    ...
    close(4396)
        ...
        f_atom_out(1:3, 1:n_atom) = f_atom_predict ! calculated by ML_FF_EF
        goto 8889 ! end
