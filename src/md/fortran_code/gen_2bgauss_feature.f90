module calc_2bgauss_feature
    ...
    integer num_step, natom, i, j,natom3,m_neigh3
...
!  FInish the initial grid treatment
    ...
    subroutine set_image_info_type3(atom_type_list,is_reset,natom_tmp)
...
subroutine gen_feature_2bgauss(AL,xatom)
    ...
    ! instead of natom. Each process only needs to know its own natom_n
        ...
        allocate(list_neigh_alltypeM3(m_neigh,natom))
        allocate(num_neigh_alltypeM3(natom))
        ...
        allocate(feat_M3(nfeat0m,natom_n))
        allocate(dfeat_M3(nfeat0m,natom_n,m_neigh,3))
