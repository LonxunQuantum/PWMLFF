...
! ii is the index position in the tensor(imu), for: [ind(iloop(1,i)),ind(iloop(2,i)),...,ind(iloop(rankT,i))]
      ...
      enddo
      feat_all(kk,iat1)=sum
      dfeat_all(kk,iat1,1:nneigh,1)=dprod_sum(1,1:nneigh)
      dfeat_all(kk,iat1,1:nneigh,2)=dprod_sum(2,1:nneigh)
      dfeat_all(kk,iat1,1:nneigh,3)=dprod_sum(3,1:nneigh)
! This is the core contraction part
...
2000  continue
      ...
      nfeat_atom_tmp(iat)=numCC_type(itype0)
      deallocate(tensor)
      deallocate(dtensor)
